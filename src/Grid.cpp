...
#pragma omp parallel default(none) shared(lhs, x, y) firstprivate(a, b, shift)
  {
#ifdef LIKWID_PERFMON
...
#pragma omp parallel default(none) shared(X, P, Z) firstprivate(xpscale, zpscale, shift)
  {
#ifdef LIKWID_PERFMON
...
// Calculates lhs[:] = a*x[:] + b*y[:]
double axpby_dot(Grid *lhs, double a, Grid *x, double b, Grid *y, bool halo)
{
  START_TIMER(AXPBY_DOT);
#ifdef DEBUG
  assert((lhs->numGrids_y(true) == x->numGrids_y(true)) && (lhs->numGrids_x(true) == x->numGrids_x(true)));
  assert((y->numGrids_y(true) == x->numGrids_y(true)) && (y->numGrids_x(true) == x->numGrids_x(true)));
#endif
...
#pragma omp parallel reduction(+ : dotprod) default(none) shared(lhs, x, y) firstprivate(a, b, shift)
  {
#ifdef LIKWID_PERFMON
    LIKWID_MARKER_START("AXPBY_DOT");
#endif
...
#pragma omp simd
      for (int xIndex = shift; xIndex < lhs->numGrids_x(true) - shift; ++xIndex)
      {
        ...
        (*lhs)(yIndex, xIndex) = temp;
        dotprod += (temp * temp);
      }
    }

#ifdef LIKWID_PERFMON
    LIKWID_MARKER_STOP("AXPBY_DOT");
#endif
  }

  STOP_TIMER(AXPBY_DOT);
  return dotprod;
}
...
#pragma omp parallel default(none) shared(lhs, rhs) firstprivate(a, shift)
  {
...
#pragma omp parallel reduction(+ : dot_res) default(none) shared(x, y) firstprivate(shift)
  {
#ifdef LIKWID_PERFMON
