...
for simrange in "2000 20000" "20000 2000" "1000 400000"; do
    ...
    echo "---------------------------------------------------------------------------------------"
    pfilename=procfile_$simdir
    for numcores in $(seq 1 $cpustart) $(seq $numacpustart $cpustep $procs); do
        echo -n "numcores $numcores $simrange : "
        srun --cpu-freq=2000000-2000000:performance likwid-pin -q -c E:N:$numcores ./$perffile ${simrange} >$pfilename
        numthreads=$(cat $pfilename | grep -i "threads active" | awk '{print $(NF)}')
        cgperf=$(cat $pfilename | grep -i "Performance CG" | awk '{print $(NF-1)}')
        pcgperf=$(cat $pfilename | grep -i "Performance PCG" | awk '{print $(NF-1)}')
        rm $pfilename
